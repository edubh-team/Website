...
import manipalLogo from "@/assets/manipal.png"; // Import the Manipal logo
...
export default function ManipalCoursesPage() {
  ...
  const manipalCourses = courses.filter(course => course.university === "Manipal University");
  const [filteredCourses, setFilteredCourses] = useState<Course[]>(manipalCourses);
  ...
  useEffect(() => {
    if (selectedCategory) {
      setFilteredCourses(manipalCourses.filter((course) => course.category === selectedCategory));
    } else {
      setFilteredCourses(manipalCourses);
    }
  }, [selectedCategory]);

  const categories = Array.from(new Set(manipalCourses.map((course) => course.category)));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFFFFF] to-[#D2DCFF] pt-32 px-8 pb-8 relative overflow-hidden" ref={heroRef}>
      ...
      <div className="flex flex-col items-center mb-4">
        <img src={manipalLogo.src} alt="Manipal Logo" className="h-16 w-auto mb-4" />
        <h1 className="text-4xl font-bold text-center">Manipal University Courses</h1>
      </div>
